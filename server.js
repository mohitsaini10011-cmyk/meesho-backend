...
app.post("/admin/login", (req, res) => {
  ...
  if (email === normEmail(ADMIN_EMAIL) && password === ADMIN_PASSWORD) {
    ...
    if (!admin) {
      ensureAdminUser();
      admin = readDB().users.find(u => u.email === email);
    }
...
app.get("/admin/users", adminRequired, (req, res) => {
  ...
  res.json({
    ...
    plans: db.plans,
    logs: db.loginLogs.slice(-100).reverse()
  });
});
...
app.post("/admin/users/update-plan", adminRequired, (req, res) => {
  ...
  user.subscription = {
    planId: plan.id,
    planName: plan.name,
    expiry: addDays(req.body.days || plan.days),
...
app.use(
  ["/generate", "/generate-from-form", "/generate-from-text", "/shipping-optimize"],
  authRequired
);
...
function buildFields(description) {
  ...
  return [
    f("product_name", makeTitle(description)),
