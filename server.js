...
function validateLicense(req, res, next) {
  // Skip license validation entirely - license feature disabled
  // All users can access the API without license key
  return next();
  
  // Old license validation code (disabled):
  /*
  if (req.path.startsWith('/admin') || req.path === '/health' || req.path === '/validate-license') {
    return next();
  }
  // ... rest of old validation
  */
}
...
app.post('/admin/generate-key', verifyAdmin, (req, res) => {
  try {
    const { plan, expiryDays } = req.body;
    const newKey = licenseManager.addKey(plan || 'monthly', expiryDays || 30);
...
app.get('/admin/keys', verifyAdmin, (req, res) => {
  try {
    const keys = licenseManager.getAllKeys();
...
app.post('/admin/deactivate-key', verifyAdmin, (req, res) => {
  try {
    const { key } = req.body;
    const result = licenseManager.deactivateKey(key);
    res.json(result);
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
...
// Admin: Get key info
app.get('/admin/key/:key', verifyAdmin, (req, res) => {
  try {
    const info = licenseManager.getKeyInfo(req.params.key);
    if (!info) {
      return res.status(404).json({ success: false, error: 'Key not found' });
    }
    res.json({ success: true, key: info });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
app.get('/admin/stats', verifyAdmin, (req, res) => {
  try {
    ...
    const expiredKeys = keys.filter(k => !k.active || new Date(k.expiry) < new Date());
